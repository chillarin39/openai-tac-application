...
TAC 有効化直後は [ai-usage-policy.md §9](docs/ai-usage-policy.md) の **Phase 1: 対話型** に留め、以下の用途に限定します:

- サイバーセキュリティ指南書の章執筆補助 (OWASP Top 10 / MITRE ATT&CK / NIST CSF 2.0)
- CVE 影響範囲調査・検知ルール (Snort / Suricata / Sigma) の妥当性検証
- セキュアコードレビュー (自リポジトリの PR レビュー)
- インシデント対応訓練 (合成ログのトリアージ)

Phase 2 以降 (Codex Security 経由の自リポジトリ監査 / 隔離セグメント内 HTTP 接続) は、3 ヶ月以上の Phase 1 運用実績 + ポリシー逸脱なしを確認したうえで着手判断します。

利用記録は [docs/usage-log.md](docs/usage-log.md) で月次運用します。
